...
def buy_stock(stock_name, volume, price, initial_balance, match_time: datetime):
    ...
    for i, q in enumerate(buysell_q):
        if q["stock"] == stock_name:
            if q["vol"] < volume:
                volume = q["vol"]
            q["vol"] -= volume
            if q["vol"] == volume:
...
def running_buy_sell(initial_balance):
    global transaction_q
    global buysell_q
    
    for q in buysell_q:
        if q["type"] == "Buy":
            create_buy_stock(q["stock"], q["vol"], q["price"])
        elif q["type"] == "Sell":
            create_sell_stock(q["stock"], q["price"])
...
def create_buy_queue(stock_name, volume, price):
    global buysell_q
    buysell_q = [q for q in buysell_q if q["stock"]!= stock_name]
    buysell_q.append({"type": "Buy", "stock": stock_name, "vol": volume, "price": price})
    return

def create_sell_queue(stock_name, price):
    global buysell_q
    buysell_q = [q for q in buysell_q if q["stock"]!= stock_name]
