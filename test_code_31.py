...
def calculate_confidence(buffer1, buffer2, volume):
    confidence = abs(buffer1 - buffer2) / (abs(buffer2) + 1e-6)  # เพิ่มค่าคงที่เล็กน้อยเพื่อเลี่ยงหารศูนย์
    confidence = confidence * volume.clip(upper=1000) / 1000  # ใช้ clip จำกัดค่า volume ให้อยู่ระหว่าง 0-1000
    return confidence.clip(upper=1)
...
for uniq in unique_sharecodes:
    ...
    eq_df[uniq]['Confidence'] = calculate_confidence(buffer1, buffer2, eq_df[uniq]['Volume'])
    eq_df[uniq]['Shares_To_Buy'] = (
        100 + (eq_df[uniq]['Confidence'] * money_per_turn / eq_df[uniq]['Close'] // 100) * 100
    )

    # แก้ไขค่าที่ไม่ใช่ตัวเลข (NaN หรือ inf)
    eq_df[uniq]['Shares_To_Buy'] = eq_df[uniq]['Shares_To_Buy'].replace([np.inf, -np.inf], np.nan)
    eq_df[uniq]['Shares_To_Buy'] = eq_df[uniq]['Shares_To_Buy'].fillna(0).astype(int)

    # กำหนดจำนวนขั้นต่ำที่ 100 หุ้น
    eq_df[uniq]['Shares_To_Buy'] = eq_df[uniq]['Shares_To_Buy'].apply(lambda x: max(x, 100))
        ...
        if itr[uniq] < len(eq_df[uniq]):
            ...
            if trade_dt == time_now.time():
                ...
                if uniq in portfolio:
                    if portfolio[uniq]['price'] >= take_profit_price and time_now.time() < time(16, 00):
